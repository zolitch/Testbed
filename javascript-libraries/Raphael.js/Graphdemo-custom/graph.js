...
(function ($) {
	...
	window.onload = function () {
		...
		var width = 600,
			...
			yValues = 12;
		r.drawGrid(leftgutter + X * .5 + .5, topgutter + .5, width - leftgutter - X, height - topgutter - bottomgutter, 8, yValues, "#333");
		var path = r.path().attr({stroke: color, "stroke-width": 2}),
			bgp = r.path().attr({stroke: "none", fill: "270-#64aa3c:0-#000", opacity: .7}),
			label = r.set(),
		...
		for (var i = 0, ii = labels.length; i < ii; i++) {
			var y = Math.round(height - bottomgutter - Y * data[i]),
				...
				t = r.text(x, height - 6, labels[i]).attr(txt).toBack(); /*The x values along the bottom*/
			if (!i) {
				p = ["M", x, y, "C", x, y];
			...
			//var invisibleDot = r.circle(x, y, 0).attr({fill: "#000", stroke: color, "stroke-width": 0});
